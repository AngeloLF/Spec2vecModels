...
class FOPASv2_Model(nn.Module):
    """
    FOPAS : Finding Ozone, PWV and Aerosols in Spectrogram
    """
    ...
    def __init__(self):
        ...
        super(FOPASv2_Model, self).__init__()
        ...
        # MLP
        self.fc1 = nn.Linear(64 * 16 * 128, 512) # 131072 -> 1024
        self.relu_fc1 = nn.ReLU()
        self.fc2 = nn.Linear(512, 128)           # 1024 -> 800
        self.relu_fc2 = nn.ReLU()
    
        self.fc3_ozone    = nn.Linear(128, 1)
        self.fc3_pwv      = nn.Linear(128, 1)
        self.fc3_aerosols = nn.Linear(128, 1)
    ...
    def forward(self, x):
        ...
        ozone = self.fc3_ozone(x)
        pwv = self.fc3_pwv(x)
        aerosols = self.fc3_aerosols(x)

        return torch.cat([ozone, pwv, aerosols], dim=1) # (batch, 3)
